...
const CartButton = ({ cart = [] }) => {
	...
	let totalPrice, buttonCartText = undefined

	if (cart.length > 0) {
		// Определяем количество пицц в заказе
		totalPrice = <div className='flex-item cart-button_count'>{cart.reduce((total, pizza) => total + pizza.price * pizza.count, 0)}&nbsp;₽</div>
